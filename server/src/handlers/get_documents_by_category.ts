...
import { type Language, type LocalizedDocument } from '../schema';
import { eq, and } from 'drizzle-orm';

export async function getDocumentsByCategory(categoryId: number, language: Language = 'id'): Promise<LocalizedDocument[]> {
  try {
    // Query legal documents joined with categories to get category name
    const results = await db.select()
      .from(legalDocumentsTable)
      .innerJoin(categoriesTable, eq(legalDocumentsTable.category_id, categoriesTable.id))
      .where(
        and(
          eq(legalDocumentsTable.category_id, categoryId),
          eq(legalDocumentsTable.is_published, true)
        )
      )
      .execute();

    // Transform results to localized format based on language
